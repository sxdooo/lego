...
import { resolveRequestUrl, validateForm } from '../src/pages/page/runtimeFormLogic.js';
...
test('resolveRequestUrl: absolute url as-is', () => {
  assert.equal(
    resolveRequestUrl('https://example.com/a', 'http://api'),
    'https://example.com/a'
  );
...
test('resolveRequestUrl: joins apiBase and relative path', () => {
  assert.equal(
    resolveRequestUrl('/api/x', 'http://localhost:4000'),
...
test('validateForm: fails required with message', () => {
  const r = validateForm(
    [{ name: 'a', required: true, requiredMessage: 'A必填' }],
    { a: '' }
  );
